...
**録画・ピッカー・項目抽出はアプリ内のブラウザで完結します。** 追加の準備は要りません。

**シナリオの実行だけは Playwright を使います。** 自動待機・iframe対応・trace・
計測タグの監視が必要なためです。`playwright-core` はブラウザを同梱しないので、
Google Chrome がインストールされていればそのまま動きます。無い場合は次で用意してください。
...
1. **シナリオ** → 「新規作成」
2. URLを入力して「開く」→ **アプリ内にブラウザが開きます**
3. 「ピッカーを開始」→ 表示されたページ上で入力したい項目をクリック
...
**Codex CLI（`codex` コマンド）のみ**に対応しています。
インストール済みであれば、AI生成の選択肢に自動で現れます。

サブプロセスは一時ディレクトリで、サンドボックスを読み取り専用にして起動します。
コーディングエージェントが手元のファイルを触りに行かないようにするためです。

> **AIを使うと、フォームの項目情報（ラベル・型）が外部に送信されます。**
> シナリオはローカル、解析は外部、という区別を理解して使ってください。
>
> サブスク認証を他アプリのバックエンドとして使えるかは**各サービスの利用規約次第**です。
> 各自で確認のうえ自己責任でお使いください。
> また CLI のフラグはバージョンで変わります。動かない場合は `codex exec --help` で
> 現行仕様を確認し、`src/ai/providers/codexCli.ts` を直してください。

プロバイダの抽象（`AIProvider`）は残してあるので、他のバックエンドを足すのは
`src/ai/index.ts` に1行加えるだけです。
